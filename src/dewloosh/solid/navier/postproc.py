...
def pproc_M_2D(size, m: int, n:int, xp: float, yp:float, 
               coeffs : nparray, D: nparray, S: nparray):
    Lx, Ly = size
    Amn, Bmn, Cmn = coeffs
...
def pproc_Mindlin_2D(size, shape : nparray, points : nparray,
    ...
    res2d = np.zeros((M * N, nLHS, nRHS, nP, 13), dtype=D.dtype)
    for m in prange(M):
        for n in prange(N):
            iMN = m * N + n
            for iRHS in prange(nRHS):
                for iP in prange(nP):
                    ...
                    for iLHS in prange(nLHS):
                        res2d[iMN, iLHS, iRHS, iP] = \
                            pproc_M_2D(size, m+1, n+1, xp, yp, 
                                       solution[iRHS, iLHS, iMN], D[iLHS], 
                                       S[iLHS])
    return res2d


@njit(nogil=True, cache=True)
def pproc_K_2D(size, m: int, n:int, x: float, y:float, Cmn : nparray, 
               D: nparray, S: nparray, loads: nparray):
    qxx, qyy = loads[:2]
    Lx, Ly = size
    D11, D12, D22, D66 = D[0, 0], D[0, 1], D[1, 1], D[2, 2]
    S44, S55 = S[0, 0], S[1, 1]
    return np.array(
        [Cmn*sin(PI*m*x/Lx)*sin(PI*n*y/Ly), 
         PI*Cmn*n*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/Ly, 
         -PI*Cmn*m*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/Lx, 
         PI**2*Cmn*m**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Lx**2, 
         PI**2*Cmn*n**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Ly**2, 
         -2*PI**2*Cmn*m*n*cos(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx*Ly), 
         PI**3*Cmn*D11*m**3*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/(Lx**3*S44) + 
         PI**3*Cmn*D12*m*n**2*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/(Lx*Ly**2*S44) + 
         2*PI**3*Cmn*D66*m*n**2*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/(Lx*Ly**2*S44) + 
         qxx*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/S44, 
         PI**3*Cmn*D12*m**2*n*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx**2*Ly*S55) + 
         PI**3*Cmn*D22*n**3*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Ly**3*S55) + 
         2*PI**3*Cmn*D66*m**2*n*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx**2*Ly*S55) + 
         qyy*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/S55, 
         PI**2*Cmn*D11*m**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Lx**2 + 
         PI**2*Cmn*D12*n**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Ly**2, 
         PI**2*Cmn*D12*m**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Lx**2 + 
         PI**2*Cmn*D22*n**2*sin(PI*m*x/Lx)*sin(PI*n*y/Ly)/Ly**2, 
         -2*PI**2*Cmn*D66*m*n*cos(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx*Ly), 
         PI**3*Cmn*D11*m**3*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/Lx**3 + 
         PI**3*Cmn*D12*m*n**2*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/(Lx*Ly**2) + 
         2*PI**3*Cmn*D66*m*n**2*sin(PI*n*y/Ly)*cos(PI*m*x/Lx)/(Lx*Ly**2) + 
         qxx*sin(PI*n*y/Ly)*cos(PI*m*x/Lx), 
         PI**3*Cmn*D12*m**2*n*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx**2*Ly) + 
         PI**3*Cmn*D22*n**3*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/Ly**3 + 
         2*PI**3*Cmn*D66*m**2*n*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)/(Lx**2*Ly) + 
         qyy*sin(PI*m*x/Lx)*cos(PI*n*y/Ly)]
    )
        ...
        for n in prange(N):
            ...
            for iRHS in prange(nRHS):
                for iP in prange(nP):
                    ...
                    for iLHS in prange(nLHS):
                        res2d[iMN, iLHS, iRHS, iP, :] = \
                            pproc_K_2D(size, m+1, n+1, xp, yp, 
                                       solution[iRHS, iLHS, iMN], D[iLHS], 
                                       S[iLHS], loads[iRHS, iMN])
    return res2d
