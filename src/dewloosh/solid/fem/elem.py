...
class FiniteElement(object):
    ...
    def stiffness_matrix(self, *args, topo=None, q: Quadrature=None, **kwargs):
        if q is None:
            ...
            if isinstance(q, dict):
                ...
                for qinds, qvalue in q.items():
                    ...
                    res[:, :, :] += self.stiffness_matrix(*args, topo=topo, q=q, **kwargs)
            else:
    ...
    def stiffness_matrix_coo(self, *args, **kwargs):
        ...
        N = nP * self.NDOFN
        topo = self.nodes.to_numpy()
        K_bulk = self.stiffness_matrix(*args, topo=topo, **kwargs)
        gnum = self.global_dof_numbering(topo=topo)
        return fem_coeff_matrix_coo(K_bulk, *args, inds=gnum, N=N, **kwargs)
